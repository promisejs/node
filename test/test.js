...
describe('TEST API', () => {
  ...
  it('should add a single event on /events/<id> POST', (done) => {
    ...
    chai.request(server).post('/events').send(eventToPost).end((err, res) => {
      res.should.have.status(200);
      res.should.be.a('object');
      res.body.should.have.property('id');
      res.body.should.have.property('title');
  ...
  it('should delete a single event on /events/<id> DELETE', (done) => {
    chai.request(server).delete('/events/' + event.id).end((err, res) => {
      res.should.have.status(200);
      res.should.be.json;
      res.body.should.have.property('value')
      res.body.value.should.equal('deleted')
      done()
    })
  })
  ...
  it('should update a single event on /events/<id> UPDATE', (done) => {
    ...
    chai.request(server).put('/events/' + event.id).send(eventToUpdate).end((err, res) => {
      res.should.have.status(200);
      res.should.be.a('object');
      res.body.should.have.property('id');
      res.body.should.have.property('title');
      ...
      res.body.should.have.property('date');
      res.body.id.should.equal(4);
      res.body.description.should.equal('Test UPDATED');
      res.body.title.should.equal('Test 5 UPDATED');
      res.body.date.should.equal('10-5-2018');
      done();
    })
  })
